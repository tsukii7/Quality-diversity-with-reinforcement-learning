...
if __name__ == "__main__":
    ...
    parser.add_argument('--config_file', type=open,
                        action=LoadFromFile)  # Config file to load args (Typically you would only specify this arg)
    ...
    ########################## QD PARAMS #####################################################################
    ##########################################################################################################
    parser.add_argument("--dim_map", default=4, type=int)  # Dimensionality of behaviour space
    ...
    parser.add_argument("--mutation_rate", default=0.05,
                        type=float)  # Probability of a gene to be mutated (Not used in GECCO paper. iso_dd mutates all genes unconditionally)
    parser.add_argument("--crossover_rate", default=0.75,
                        type=float)  # Probability of genotypes being crossed over (Not used in GECCO paper. iso_dd crosses all genes unconditionally)
    parser.add_argument("--eta_m", default=5.0,
                        type=float)  # Parameter for polynomial mutation (Not used in GECCO paper)
    ...
    parser.add_argument("--sigma", default=0.2,
                        type=float)  # Standard deviation for gaussian mutation (Not used in GECCO paper)
    ...
    parser.add_argument("--max_uniform", default=0.1,
                        type=float)  # Max mutation for uniform mutation (Not used in GECCO paper)
    ...
    parser.add_argument("--random_init", default=500, type=int)  # Number of random evaluations to initialise (G)
    ...
    parser.add_argument("--use_cached_cvt",
                        action="store_true")  # Use cached centroids for creating archive if available
    parser.add_argument("--not_discard_dead",
                        action="store_true")  # Don't discard solutions that does not survive the entire simulation (Set to not discard in GECCO paper)
    ...
    parser.add_argument('--nr_of_steps_crit', default=300,
                        type=int)  # Nr of. training steps for critic training (n_crit)
    parser.add_argument('--nr_of_steps_act', default=10, type=int)  # Nr of. training steps for PG variation (n_grad)
    ...
    parser.add_argument("--affine",
                        action="store_true")  # Use affine transformation with layer norm (Not used in GECCO paper)
    ...
    # k-nn for archive addition. The nearest centroid is found by this by setting k=1.
